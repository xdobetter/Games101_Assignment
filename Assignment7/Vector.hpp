...
inline Vector3f normalize(const Vector3f &v)//归一化,a/|a|
{
...
inline float distance(const Vector3f &a,const Vector3f &b){
    float mag2=(a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y)+(a.z-b.z)*(a.z-b.z);
    return sqrt(mag2);
}
inline float distance_2(const Vector3f &a,const Vector3f &b){
    float mag2=(a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y)+(a.z-b.z)*(a.z-b.z);
    return mag2;
}
