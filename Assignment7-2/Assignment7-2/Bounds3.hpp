...
//    for (int i = 0; i < 3; i++) {
//        float t_near, t_far;
//        t_near = (pMin[i] - ray.origin[i]) * invDir[i];
//        t_far = (pMax[i] - ray.origin[i]) * invDir[i];
//        if (dirIsNeg[i]==0) std::swap(t_near, t_far);
//        t_enter = std::max(t_near, t_enter);
//        t_out = std::min(t_far, t_out);
//    }
//    if (t_enter < t_out && t_out > 0) return true;
//    return false;
