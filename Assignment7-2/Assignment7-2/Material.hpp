...
#include "Vector.hpp"
#include <algorithm>
enum MaterialType { DIFFUSE,SPECULAR,MICROFACET,PHONG,BLING_PHONG};
...
Vector3f Material::sample(const Vector3f &wi, const Vector3f &N){//采一根光路
    switch(m_type){
        case DIFFUSE:
        case SPECULAR:
        case MICROFACET:
...
Vector3f Material::cookTorrance(const Vector3f &wi, const Vector3f &wo, const Vector3f &N) {
    ...
    auto getFresnel = [&]() {
        ...
        double n1 = n_air, n2;
        if(type == MaterialType::MICROFACET) n2 = n_diff;
        else n2 = n_glos;
    ...
    }
    {
        ...
        if(type == MaterialType::MICROFACET) m = 0.6;
        else m = 0.2;
    ...
    switch(m_type){
        case DIFFUSE:
        {
            ...
            if (cosalpha > 0.0f) {
                ...
                Vector3f diffuse=Kd* eval_diffuse(wi,wo,N);
                return diffuse;
            }
            else
        ...
        case SPECULAR:
        {
            ...
            if(cosalpha>0.0f){
                ...
                auto L=wi;
                auto H= normalize(V+L);
                float spec=std::pow(std::max(dotProduct(V,H),0.0f),32);
